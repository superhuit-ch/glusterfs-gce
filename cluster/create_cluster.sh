...
for (( i=1; i<=${COUNT}; i++ ))
do
	gcloud compute disks create ${DISK}1-${i} --size ${SIZE} \
	 --type pd-standard --zone ${REGION}-${ZONES[$i-1]}
done
...
do
	...
	until gcloud compute instances describe \
	--zone ${REGION}-${ZONES[$i-1]} ${SERVER}-${i} | \
	grep "RUNNING" >/dev/null 2>&1; do a=$(( (a+1) %4 )); \
	printf "\r${spin:$a:1}"; sleep .1; done

	sleep 20 # hacky
	VM_EXT_IP=$(gcloud compute instances list | grep -v grep | \
	grep ${SERVER}-${i}| awk {'print $5'})
	...
	gcloud compute ssh --zone ${REGION}-${ZONES[$i-1]} ${SERVER}-${i} --command \
	"sudo file -sL /dev/sdb | grep XFS || sudo mkfs.xfs -i size=512 /dev/sdb"
	echo " "
	...
	gcloud compute ssh --zone ${REGION}-${ZONES[$i-1]} ${SERVER}-${i} --command \
	"yes | sudo apt-get install glusterfs-server"
	echo " "
...
do
	...
	gcloud compute ssh --zone ${REGION}-${ZONES[0]} ${SERVER}-1 --command \
	"sudo gluster peer probe ${SERVER}-${i}"
done
